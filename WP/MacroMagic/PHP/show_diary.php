<?php
	...
	echo "<div id = 'date_buttons'><button id = 'subtract_day' onclick = 'subtract_day()' >Previous Day</button>
	" . " " . $date->format('F j, Y') . " " . "
	<button id = 'add_day' onclick = 'add_day()'>Next Day</button></div><div id = 'food_journal_notify'><br></div>";
	...
	while($result && $row = mysqli_fetch_array($result)) 
	{
		...
		if($result_nut)
		{
			$row_nut = mysqli_fetch_row($result_nut);
			if($row_nut == NULL)
			{
				echo "<td>" . "DB Fetch Fail " . "</td>";
			}else
			{
				echo "<td>" . number_format(($row['weight']/100)*$row_nut[0], 0) . "</td>";
			}
		}else
		{
			echo "<td>0</td>";
